...
contract BindingPolicy {
    ...
    function findRole(address actor, uint rActor, address pCase) public view returns(bool); // Pending
...
contract BindingAccessControl {
    ...
    function linkRoleToTask(address pCase, uint roleInd, uint taskInd) public {
        if(caseIndex[pCase] == 0)
            caseIndex[pCase] = caseCount++;
        taskRoleMap[caseIndex[pCase]][taskInd] = roleInd;
    }
    ...
    function canPerform(address actor, address pCase, uint taskIndex) public view returns(bool) {
        ...
        uint tRole = taskRoleMap[caseIndex[pCase]][taskIndex];
        if(tRole == 0)
            tRole = ControlFlow(taskRoleAdr).getRoleFromTask(taskIndex, pId);
        return findRole(actor, tRole, pCase);
    }
