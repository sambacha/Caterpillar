...
let contractsInfo;
let processInfo;
let deployedContracts = new Map();
let pendingTrans = new Map();
...
models.post('/interpreter/models', (req, res) => {
    ...
    BPMNParser_1.parseBpmnModel(req.body.bpmn)
        .then((procInfo) => {
...
models.post('/i-flow/element/:eInd', (req, res) => {
    ...
    let eInd = +req.params.eInd;
    let eInfo = processInfo.iflow.elementInfo.get(eInd);
...
let joinTypes = (eInd) => {
    let params = processInfo.iData.inParams.get(eInd);
    let res = 'uint256';
...
let instantiateContract = (key) => {
    return new Promise((resolve, reject) => {
        let contract = web3.eth.contract(JSON.parse(contractsInfo[key].interface));
        let bytecode = contractsInfo[key].bytecode;
        let gasEstimate = web3.eth.estimateGas({ data: bytecode });
        contract.new({ from: web3.eth.accounts[0], data: "0x" + bytecode, gas: gasEstimate + 50000 }, (errF, contractF) => {
            if (errF) {
