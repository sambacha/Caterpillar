    ...
    function step(uint tmpMarking, uint tmpStartedActivities) internal {
        while (true) {
            if (tmpMarking & uint(2) != 0) {
                Invoice_Handling_AbstractWorklist(worklist).Issue_Invoice_start(1);
                tmpMarking &= uint(~2);
                tmpStartedActivities |= uint(2);
                continue;
            }
            if (tmpMarking & uint(8) != 0) {
                Invoice_Handling_AbstractWorklist(worklist).Approve_Invoice_start(2);
                tmpMarking &= uint(~8);
...
if (accepted)                tmpMarking |= uint(32);
            ...
            if (tmpMarking & uint(64) != 0) {
                Invoice_Handling_AbstractWorklist(worklist).Fix_Invoice_start(6);
                tmpMarking &= uint(~64);
...
contract Invoice_Handling_AbstractWorklist {
...
contract Invoice_Handling_worklist is AbstractWorklist {
    ...
    function Issue_Invoice_start(uint elementIndex) external {
        ...
        emit Issue_Invoice_Requested(workitems.length - 1);
    }
    function Approve_Invoice_start(uint elementIndex) external {
        workitems.push(Workitem(elementIndex, msg.sender));
        emit Approve_Invoice_Requested(workitems.length - 1);
    }
    function Fix_Invoice_start(uint elementIndex) external {
        workitems.push(Workitem(elementIndex, msg.sender));
        emit Fix_Invoice_Requested(workitems.length - 1);
    }
    ...
    function Issue_Invoice(uint workitemId) external {
        ...
        Invoice_Handling_AbstractWorklist(workitems[workitemId].processInstanceAddr).Issue_Invoice_complete(workitems[workitemId].elementIndex);
    ...
    function Approve_Invoice(uint workitemId, bool _accepted) external {
        ...
        Invoice_Handling_AbstractWorklist(workitems[workitemId].processInstanceAddr).Approve_Invoice_complete(workitems[workitemId].elementIndex, _accepted);
        workitems[workitemId].processInstanceAddr = address(0);
    }
    function Fix_Invoice(uint workitemId) external {
        ...
        Invoice_Handling_AbstractWorklist(workitems[workitemId].processInstanceAddr).Fix_Invoice_complete(workitems[workitemId].elementIndex);
        workitems[workitemId].processInstanceAddr = address(0);
    }
