...
const bpmn2solIDataEJS = fs.readFileSync(path.join(__dirname, "./../../../../templates-ejs/interpretation-engine") +
    "/bpmn2solIData.ejs", "utf-8");
const factory2solEJS = fs.readFileSync(path.join(__dirname, "./../../../../templates-ejs/interpretation-engine") +
    "/factory2Solidity.ejs", "utf-8");
...
let parseBpmn = async (bpmnDoc) => {
    return new Promise((resolve, reject) => {
        try {
            ...
            console_log_1.print("Invalid Process Model in the Input", console_log_1.TypeMessage.error);
            reject(err);
        }
    });
};
exports.parseBpmnModel = (bpmnModel) => {
    return new Promise((resolve, reject) => {
        parseBpmn(bpmnModel).then((definitions) => {
...
let buildProcessTree = (proc, instCount, eventSubProc) => {
    let processNode = new parsing_info_1.SubProcessInfo(instCount);
    processNode.procName = getNodeName(proc);
    processNode.procBpmnId = proc.id;
    let iFlow = new parsing_info_1.IFlowInfo();
    let iData = new parsing_info_1.IDataInfo();
    let arcCount = 1;
    let elemCount = 1;
    let preCMap = new Map();
            ...
            case "bpmn:SequenceFlow": {
                ...
                if (element.conditionExpression) {
                    ...
                    iData.gatewayScripts
                        .get(prevInd)
                        .push(new parsing_info_1.EdgeScript(iFlow.edgeIndexMap.get(element.id), element.conditionExpression.body));
                }
                break;
            }
            case "bpmn:UserTask": {
    ...
    iFlow.nodeIndexMap.forEach((eInd, eId) => {
        let elementInfo = new parsing_info_1.ElementIFlow(eInd, eNames.get(eInd));
...
let updateGlobalFields = (cad, iData) => {
    ...
    arr.forEach((gField) => {
        if (gField.trim().length > 0) {
            ...
            iData.globalFields.push(new parsing_info_1.ParamInfo(arr1[0], arr1[arr1.length - 1]));
        }
    });
};
...
let updateParameters = (cad, eInd, iData) => {
    ...
    for (let i = 0; i < toIterate.length; i += 2)
        inParameters.push(new parsing_info_1.ParamInfo(toIterate[i], toIterate[i + 1]));
    ...
    for (let i = 0; i < toIterate.length; i += 2)
        outParameters.push(new parsing_info_1.ParamInfo(toIterate[i], toIterate[i + 1]));
