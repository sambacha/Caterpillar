...
const compilation_info_1 = require("../utils/structs/compilation-info");
const console_log_2 = require("../../adapters/messages/console-log");
const bpmn2solEJS = fs.readFileSync(path.join(__dirname, "./../../../templates-ejs/compilation-engine") +
    "/bpmn2sol.ejs", "utf-8");
let bpmn2solTemplate = ejs.compile(bpmn2solEJS);
const workList2solEJS = fs.readFileSync(path.join(__dirname, "./../../../templates-ejs/compilation-engine") +
    "/workList2sol.ejs", "utf-8");
...
let parseBpmn = async (bpmnDoc) => {
    return new Promise((resolve, reject) => {
        try {
            ...
            console_log_2.print("Invalid Process Model in the Input", console_log_2.TypeMessage.error);
            reject(err);
        }
    });
};
...
collectControlFlowInfo = (proc, globalNodeMap, globalControlFlowInfo) => {
    ...
    if (nonBlockingBoundaryEvents.length > 0) {
        ...
        controlFlowInfo = new compilation_info_1.ControlFlowInfo(proc, mainPathNodeList, mainPathEdgeList, sources, boundaryEvents);
        ...
        for (let eventId of nonBlockingBoundaryEvents) {
            ...
            let childControlFlowInfo = new compilation_info_1.ControlFlowInfo(event, localNodeList, localEdgeList, [eventId], []);
            childControlFlowInfo.parent = proc;
            globalControlFlowInfo.push(childControlFlowInfo);
        }
    }
    else {
        controlFlowInfo = new compilation_info_1.ControlFlowInfo(proc, nodeList, edgeList, sources, boundaryEvents);
        globalControlFlowInfo.push(controlFlowInfo);
    }
...
let extractParameters = (cad, nodeId, controlFlowInfo) => {
    ...
    if (controlFlowInfo != null) {
        ...
        for (let i = 0; i < toIterate.length; i += 2)
            inParameters.push(new compilation_info_1.ParameterInfo(toIterate[i], toIterate[i + 1]));
        toIterate = resMap.get("output");
        let parameters = new Map();
        parameters.set("input", inParameters);
        parameters.set("output", outParameters);
        for (let i = 0; i < toIterate.length; i += 2)
            outParameters.push(new compilation_info_1.ParameterInfo(toIterate[i], toIterate[i + 1]));
        if (oracle_Data.length > 0) {
            ...
            if (!controlFlowInfo.oracleInfo.has(splitResult[0])) {
                controlFlowInfo.oracleInfo.set(splitResult[0], new compilation_info_1.OracleInfo(splitResult[0]));
            }
...
exports.parseModel = (modelInfo) => {
    console_log_2.print("Parsing Process Model ...", console_log_2.TypeMessage.pending);
    return new Promise((resolve, reject) => {
        parseBpmn(modelInfo.bpmn)
            .then((definitions) => {
            modelInfo.solidity = "pragma solidity >= 0.4.25 < 0.7.0;\n";
                ...
                if (proc.$type === "bpmn:Collaboration") {
                    for (let i = 0; i < definitions.rootElements.length; i++)
                        if (definitions.rootElements[i].$type === "bpmn:Process") {
                            ...
                            modelInfo.name = proc.name
                                ? proc.name.replace(/\s+/g, "_")
                                : proc.id;
                            modelInfo.id = proc.id;
                            break;
                        }
            ...
            for (let controlFlowInfo of globalControlFlowInfo) {
                if (!controlFlowInfo.isEmbedded) {
                    let multiinstanceActivities = [], callActivities = [], nonInterruptingEvents = [], catchingMessages = [];
                    ...
                    globalNodeMap.forEach((node) => {
                        if (is(node, "bpmn:SubProcess") &&
                            ...
                            controlFlowInfo.nodeList.indexOf(node.id)) {
                    ...
                    let codeGenerationInfo = {
                                        ...
                                        if (globalControlFlowInfoMap.has(node.id)) {
                                            ...
                                            localC.nodeList.forEach((elemId) => {
                                                ...
                                                if (elem.eventDefinitions &&
                                                    is(elem.eventDefinitions[0], "bpmn:TerminateEventDefinition") &&
                                                    elem.$parent.id === node.$parent.id)
                                                    res.push(node.id);
                                            });
                                        }
                                        else {
                        ...
                        getProcessCandidatesMaskFrom: (evtId, evtType, evtCode, sourceProcesses, allEvents) => {
                            ...
                            allEvents.forEach((nodeId) => {
                                ...
                                if (evtType === cad.substring(5, cad.length - 15) &&
                                    evtCode === getNodeName(globalNodeMap.get(nodeId)))
                                    ...
                                    if (res.length == 0) {
                                        eventList.forEach((nodeId) => {
                                            ...
                                            if (!eventFound &&
                                                ...
                                                node.$parent.triggeredByEvent &&
                                                node.$parent.$parent.id === parent.id) {
                        ...
                        getWorkItemsGroupByParameters: (isInput) => {
                            ...
                            controlFlowInfo.nodeList.forEach((nodeId) => {
                                ...
                                if (is(node, "bpmn:UserTask") ||
                                    ...
                                    catchingMessages.indexOf(nodeId) >= 0) {
                                    let params = "";
                                    if (node.documentation &&
                                        node.documentation[0].text &&
                                        node.documentation[0].text.length > 0 &&
                                        extractParameters(node.documentation[0].text, nodeId, null) !== undefined) {
                        ...
                        getContracts2CallArray: (subprocId, candidates) => {
                            ...
                            for (let i = 1; i < candidates.length; i++)
                                res +=
                                    ", uint(" + globalNodeIndexMap.get(candidates[i]) + ")";
                            return res + "]";
                        },
                        getPossibleKillSubprocess: () => {
                            ...
                            controlFlowInfo.boundaryEvents.forEach((nodeId) => {
                                let node = globalNodeMap.get(nodeId);
                                if (node.$parent.triggeredByEvent &&
                                    node.$parent.$parent.id !== controlFlowInfo.self.id) {
                                    if (node.isInterrupting != false &&
                                        res.indexOf(node.$parent.$parent.id) < 0)
                                        res.push(node.$parent.$parent.id);
                                }
                                else if (node.attachedToRef) {
                                    let attachedTo = node.attachedToRef.id;
                                    if (node.cancelActivity != false &&
                                        res.indexOf(attachedTo) < 0) {
                                        res.push(attachedTo);
                                    }
                                }
                            });
                                        ...
                                        if (globalControlFlowInfoMap.has(node.id)) {
                                            ...
                                            localC.nodeList.forEach((elemId) => {
                                                ...
                                                if (elem.eventDefinitions &&
                                                    is(elem.eventDefinitions[0], "bpmn:TerminateEventDefinition") &&
                                                    elem.$parent.id === node.$parent.id &&
                                                    controlFlowInfo.nodeList.indexOf(node.$parent.id) >=
                                                        0 &&
                                                    res.indexOf(node.$parent.id) < 0 &&
                                                    node.$parent.id != controlFlowInfo.self.id) {
                            ...
                            controlFlowInfo.nodeList.forEach((nodeId) => {
                                ...
                                if (node.eventDefinitions &&
                                    is(node.eventDefinitions[0], "bpmn:TerminateEventDefinition")) {
                                    if (res.indexOf(node.$parent.id) < 0 &&
                                        node.$parent.id != controlFlowInfo.self.id &&
                                        !is(globalNodeMap.get(controlFlowInfo.self.id), "bpmn:BoundaryEvent")) {
                                        res.push(node.$parent.id);
                                    }
                                }
                            });
                        ...
                        isInterrupting: (eventId) => {
                            ...
                            if (node.eventDefinitions &&
                                is(node.eventDefinitions[0], "bpmn:ErrorEventDefinition"))
                                return true;
                            if (is(node, "bpmn:StartEvent") &&
                                node.$parent &&
                                globalNodeMap.get(node.$parent.id).triggeredByEvent)
                        ...
                        subprocessNodeFullMarking: (subprocId) => {
                            ...
                            controlFlowInfo.nodeList.forEach((nodeId) => {
                                let node = globalNodeMap.get(nodeId);
                                if (is(node, "bpmn:SubProcess") ||
                                    callActivities.indexOf(node.id) >= 0 ||
                                    (nonInterruptingEvents.indexOf(node.id) >= 0 &&
                                        !is(node, "bpmn:StartEvent"))) {
                                    while (node.$parent) {
                                        if (node.$parent.id === subprocId) {
                                            if (multiinstanceActivities.indexOf(nodeId) >= 0 ||
                                                callActivities.indexOf(node.id) >= 0 ||
                                                nonInterruptingEvents.indexOf(node.id) >= 0) {
                                                bitarray[globalNodeIndexMap.get(nodeId)] = 1;
                                            }
                            ...
                            controlFlowInfo.nodeList
                                ...
                                .forEach((node) => {
                                if (node.$parent &&
                                    children.indexOf(node.$parent.id) >= 0) {
                                    bitarray[globalNodeIndexMap.get(node.id)] = 1;
                                }
                            });
                            catchingMessages
                                ...
                                .forEach((evt) => {
                                if (evt.attachedToRef &&
                                    children.indexOf(evt.attachedToRef) >= 0) {
                            ...
                            return result === "0b"
                                ...
                                : new bignumber_js_1.default(result).toFixed();
                        },
                        ...
                        subprocessFullMarking: (subprocId) => {
                            ...
                            controlFlowInfo.nodeList.forEach((nodeId) => {
                                let node = globalNodeMap.get(nodeId);
                                if (is(node, "bpmn:SubProcess") &&
                                    multiinstanceActivities.indexOf(nodeId) < 0) {
                                    while (node.$parent) {
                                        if (node.$parent.id === subprocId) {
                        ...
                        nodeParameters: (nodeId) => {
                            ...
                            if (node.documentation &&
                                ...
                                return resDict !== undefined
                                    ? resDict.get("input").length > 0 ||
                                        resDict.get("output").length > 0
                                    : false;
                            }
                            return false;
                        },
                    ...
                    controlFlowInfo.nodeList.forEach((nodeId) => {
                        ...
                        if (is(node, "bpmn:UserTask") || is(node, "bpmn:ReceiveTask")) {
                            ...
                            if (controlFlowInfo.localParameters.has(nodeId) &&
                                (controlFlowInfo.localParameters.get(nodeId).get("input")
                                    .length > 0 ||
                                    controlFlowInfo.localParameters.get(nodeId).get("output")
                                        .length > 0)) {
            ...
            //////////////////////////////////////////////////////////////////////////////////
            modelInfo.entryContractName =
                modelInfo.name +
                    ":" +
                    (proc.name ? proc.name.replace(/\s+/g, "_") : proc.id) +
                    "Workflow";
            console_log_2.print("Smart Contracts Generated From Process Model", console_log_2.TypeMessage.success);
            console_log_1.printSeparator();
            resolve();
        })
            .catch((err) => {
            console_log_2.print("Error parsing the process model", console_log_2.TypeMessage.error);
            console_log_2.print(err.toString(), console_log_2.TypeMessage.error);
            reject();
        });
    });
};
