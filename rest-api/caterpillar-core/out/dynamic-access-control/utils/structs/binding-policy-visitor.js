...
const policy_info_1 = require("./policy-info");
class BindingVisitor {
    constructor() {
        this._names = [];
        this.policy = new policy_info_1.Policy();
    }
    defaultResult() {
        return "";
    }
    ...
    visitUnbinding_statement(ctx) {
        let statement = new policy_info_1.Statement();
    ...
    createDisjunctionSet(setStr) {
        let disjunctionSet = new policy_info_1.DisjunctionSet();
        disjunctionSet.isNegative = setStr[0] === "+" ? false : true;
        let conjS = setStr.substr(1).split("|");
        conjS.forEach((value) => {
            let conjuntionSet = new policy_info_1.ConjunctionSet();
...
//# sourceMappingURL=binding-policy-visitor.js.map