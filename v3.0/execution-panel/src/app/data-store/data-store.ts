...
export class ProcessStorage {
  ...
  sendResourceModel() {
    if (this.resourceModel.length < 1)
      alert(
        "Resource File cannot be empty. Be sure to upload the required file."
      );
    else {
      this.http
        .post("http://localhost:3000/resources/policy", {
  ...
  createTaskRole(rootProcess, policyId) {
    if (rootProcess.length < 1) alert("Root proces Id cannot be empty.");
    else {
      this.http
        .post("http://localhost:3000/resources/task-role", {
          rootProc: rootProcess,
          policyId: policyId,
        })
        .subscribe(
          (resp) => {
  ...
  findRoleState(role, pCase) {
    this.http.get(`http://localhost:3000/resources/${role}/${pCase}`).subscribe(
      (resp) => {
  ...
  nominate(rNominator, rNominee, nominator, nominee, pCase) {
    this.http
      .post("http://localhost:3000/resources/nominate", {
        rNominator: rNominator,
        rNominee: rNominee,
        nominator: nominator,
        nominee: nominee,
        pCase: pCase,
  ...
  release(rNominator, rNominee, nominator, pCase) {
    this.http
      .post("http://localhost:3000/resources/release", {
        rNominator: rNominator,
        rNominee: rNominee,
        nominator: nominator,
        pCase: pCase,
  ...
  vote(
    ...
    this.http
      .post("http://localhost:3000/resources/vote", {
        rNominator: rNominator,
        rNominee: rNominee,
        rEndorser: rEndorser,
        endorser: endorser,
  ...
  createProcessRegistry() {
    this.http.post("http://localhost:3000/registry", {}).subscribe(
      (resp) => {
  ...
  searchRegisteredModel(modelId: string) {
    this.http.get("http://localhost:3000/models").subscribe(
      (resp) => {
        const resJ = resp.json();
        if (modelId === "") {
          this.processes = resJ;
          ...
          resJ.forEach((element) => {
            if (
              (element.name.indexOf(modelId) >= 0 ||
                element.id.indexOf(modelId) >= 0) &&
              this.processes.indexOf(element) < 0
            ) {
  ...
  updateModels() {
    this.http.get("http://localhost:3000/models").subscribe((resp) => {
      this.processes = [];
  ...
  createInstance(procId: string, caseCreator: string, creatorRole: string) {
    this.http
      .post("http://localhost:3000/models/" + procId, {
  ...
  updateInstances(procId: string) {
    this.http.get("http://localhost:3000/processes/").subscribe(
      (resp) => {
        const res = resp.json();
        this.instances[procId] = [];
        res.forEach((element) => {
  ...
  set ActInst(nActInst: string) {
    this.actInst = nActInst;
  }
