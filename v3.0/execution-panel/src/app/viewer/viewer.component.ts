...
export class ViewerComponent implements OnInit {
  ...
  updateContracts() {
    ...
    if (
      ...
      this.activeContracts[0] === "No Contracts Available"
    ) {
  ...
  setupListeners() {
    ...
    eventBus.on("element.click", (e: any) => {
      ...
      if (workItem) {
        ...
        let inputParam = [];
        if (workItem.input.length > 0) inputParam = workItem.input;
        this.pCases = workItem.pCases;
        const overlayHtml = jQuery(
          input_params_as_form({
            nodeId: workItem.elementId,
            inputs: workItem.input,
            pCases: workItem.pCases,
            href: workItem.hrefs,
          })
        );

        overlays.add(workItem.elementId, {
          position: { bottom: 0, right: 0 },
          html: overlayHtml,
        });
        overlayHtml.find(`#${workItem.elementId}_save`).click((e: any) => {
          const nodeId1 = e.target.id.slice(0, e.target.id.indexOf("_save"));
          overlays.remove({ element: nodeId1 });

          const children = e.target.parentElement.querySelectorAll(
            ".form-control"
          );
              ...
              workItem.input.forEach((input: any) => {
                children.forEach((child: any) => {
                  if (child.id === input.name) {
                    values.push(
                      input.type === "bool" ? child.checked : child.value
                    );
                  }
                });
              });
          ...
          this.http
            .post("http://localhost:3000" + workItem.hrefs[workItemIndex], {
            ...
            .subscribe((resp) =>
              this.http
                .get(this.url)
                .subscribe((resp1) => this.renderState(resp1.json()))
            );
        });
